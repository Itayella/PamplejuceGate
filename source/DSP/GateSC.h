...
public:
    ...
    void process (const ProcessContext& context, const ProcessContext& contextSC) noexcept
    {
        //if(! contextSC == nullptr)
            
        const auto& inputBlock = context.getInputBlock();
        const auto& scInputBlock = contextSC.getInputBlock();
        ...
        for (size_t channel = 0; channel < numChannels; ++channel)
        {
            ...
            auto* scInputSamples  = scInputBlock .getChannelPointer (channel);
            auto* outputSamples = outputBlock.getChannelPointer (channel);

            for (size_t i = 0; i < numSamples; ++i)
                outputSamples[i] = processSample ((int) channel, inputSamples[i], scInputSamples[i]);
        }
    }

    /** Performs the processing operation on a single sample at a time. */
    SampleType processSample (int channel, SampleType sample, SampleType sampleSC = 0);
